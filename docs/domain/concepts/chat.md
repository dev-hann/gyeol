...
| 도구 | 기능 |
|------|------|
| `assign_worker` | 워커를 레이어에 배정 |
| `unassign_worker` | 워커를 레이어에서 해제 |
...
| `get_queue_status` | 태스크 큐 크기 및 시스템 요약 |
| `arrange_layers` | 레이어 위치 초기화 (자동 레이아웃) |

### 모니터링
| 도구 | 기능 |
|------|------|
| `list_logs` | 실행 로그 조회 (태스크 ID 필터링 가능) |
| `list_tasks` | 처리 큐의 태스크 목록 |
| `get_worker_details` | 워커 상세 정보 (최근 로그 포함) |

### 프로바이더 관리
| 도구 | 기능 |
|------|------|
| `list_providers` | 전체 AI 제공자 및 사용 가능 모델 조회 |
| `switch_provider` | 활성 AI 제공자 전환 |

### 대화 관리
| 도구 | 기능 |
|------|------|
| `rename_conversation` | 대화 제목 변경 |
| `search_messages` | 키워드로 메시지 검색 |
| `clear_conversation` | 대화 메시지 전체 삭제 |
| `export_conversation` | 대화를 마크다운으로 내보내기 |

### 사용자 상호작용
| 도구 | 기능 |
|------|------|
| `present_choices` | 선택지를 클릭 가능한 버튼으로 표시 |
| `confirm_action` | 파괴적 작업 전 확인 대화상자 표시 |
...
**히스토리 정리**: API에 전달하기 전 최근 30개 메시지로 자릅니다. `tool` 역할 메시지와 그에 대응하는 빈 `assistant` 메시지(도구 호출 직전의 빈 응답)는 함께 제거하여 LLM API 오류를 방지합니다. 연속된 같은 역할의 메시지는 줄바꿈으로 병합되며, 히스토리 마지막 메시지와 새 사용자 메시지도 같은 역할이면 병합됩니다.
